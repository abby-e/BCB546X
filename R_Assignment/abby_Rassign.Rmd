...
Check that the working directory is the R_Assignment folder.
```{r}
getwd()
```
...
I got a comment that the below line of code would not run, possibly due to an issue that my above library(tidyverse) command was not functioning properly, but I have been able to run it with no issues and after closing and reopening my R environment to clear loaded packages.I self-confirmed that my reviewer likely did not run the load tidyverse command above, because I was able to recreate the error by trying the command below before loading tidyverse in a fresh R environment.
...
teosinte2 %>% arrange(Chromosome, desc(Position)) %>% filter(Chromosome == 8) %>% write.table("teosinte_chr8_rev.txt", sep="\t",col.names = FALSE, row.names = FALSE, quote = FALSE)
...
The next two plot the proportion of heterozygous, homozygous, and missing samples on each chromosome.
```{r}
...
The next two plots show the proportion of heterozygous, homozygous, and missing SNPs in each sample.
```{r}
...
>>>>>>> 7ddbec5ad2fc47e140c40c4196e2ac0099a4e0bc
